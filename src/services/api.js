...
const apiService = {
  ...
  async getConsultationTrend() {
    ...
    // Map consultationOverTime to expected format
  ...
  async getPrescriptionTrend() {
    ...
    // Map prescriptionVolumeTrend to expected format
  ...
  async getActiveDoctors() {
    ...
    // Map active_doctors_vs_patients to expected format
    const categories = result.data.active_doctors_vs_patients.categories;
    const doctorsSeries = result.data.active_doctors_vs_patients.series.find(s => s.name === "Active Doctors");
    const patientsSeries = result.data.active_doctors_vs_patients.series.find(s => s.name === "Active Patients");
  ...
  async getSpecializations() {
    ...
    // Map top_specialities_in_demand to expected format
  ...
  async getRecentPatients() {
    ...
    // Map patients data to expected format
    return result.data.data.map((patient) => ({
      ...
      lastVisit: patient.last_seen ? patient.last_seen.replace('T', ' ').split('.')[0] : 'N/A',
      location: patient.state || patient.user?.state || 'N/A',
      device: patient.user?.devices?.[0]?.platform === 'android' ? 'Android' : 
              patient.user?.devices?.[0]?.platform === 'ios' ? 'iOS' : 'N/A',
      status: patient.user?.email_verified_at || patient.user?.phone_verified_at ? 'Verified' : 'Pending'
    }));
  },
};
